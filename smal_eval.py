...
flags.DEFINE_boolean("output_measure",False,"Outputs the IOU,Overlap and PCK to CSVs")
...
def get_bbox(img_path, kp):
    # Load the mask
    mask_img = np.asarray(imageio.imread(img_path.replace('images', 'bgsub'))) / 255.
    # Load the image
    img = np.asarray(imageio.imread(img_path)) / 255.
...
def preprocess_image(img_path, img_size=256, kp=None, border=5, bgval=-1, img=None, img_ext='.jpg'):
    '''
    Performs pre-processing on the image
    '''
    if img is None:
        img = np.asarray(imageio.imread(img_path)) / 255.
    ...
    if bgval == -1:
        ...
        if osp.exists(full_img_path):
            full_img = np.asarray(imageio.imread(full_img_path))/255.
...
def visualize_opt(img, predictor, renderer, data, out_path):
    '''
    Visualises and outputs the optimisations
    '''
...
def visualize(img, outputs, renderer, img_path=None, opts=None, cam_gt=0,
    ...
    imageio.imwrite(osp.join(opts.out_path, 'shape_ov_'+out_path), Iov)
    imageio.imwrite(osp.join(opts.out_path, 'shape_'+out_path), I)

    N = 0.6*np.abs(tex_mask[:,:,:3]-1)
    if 'texture' in outputs.keys():
        imageio.imwrite(osp.join(opts.out_path, 'tex_'+out_path), N+T*tex_mask[:,:,:3])
        imageio.imwrite(osp.join(opts.out_path, 'img_'+out_path), img_pred)
...
def save_params(outputs, idx):
    ''' 
    Saves parameter data for an image model
    '''
...
def main(_):
    ...
    for iidx, img_path in enumerate(images):
        if use_annotations:
            if opts.synthetic:
                ...
                res = pkl.load(open(anno_path,"rb"),fix_imports=True, encoding="latin1")
                kp = res['keypoints']
                pose = res['pose']
                trans = res['trans']
                flength = res['flength']
        ...
        if opts.save_input:
            imageio.imwrite(osp.join(opts.out_path, 'proc_'+osp.basename(img_path)), 255*np.transpose(img, (1, 2, 0)))
            if segm_eval:
                imageio.imwrite(osp.join(opts.out_path, 'mask_proc_'+osp.basename(img_path)), 255*np.transpose(mask_img, (1, 2, 0)))
        ...
        if opts.test_optimization_results:
            ...
            data = pkl.load(open(res_file,"rb"),fix_imports=True, encoding="latin1")
            mask_pred = np.asarray(imageio.imread(mask_file))/255. 
        ...
        if show and not opts.test_optimization_results:
            ...
            visualize(img, outputs, predictor.vis_rend, img_path, opts,
                    kp_gt=kp, kp_pred=kp_pred, vis=vis, out_path=osp.basename(img_path)+'_test_%03d' % (idx) +'.png', tex_mask=tex_mask,
                    visualize=opts.visualize)

        if show and opts.test_optimization_results:
            visualize_opt(img, predictor, predictor.vis_rend, data, osp.basename(img_path)+'_opt_%03d' % (idx) +'.png')
        ...
        if mirror:
            ...
            if opts.save_input:
                imageio.imwrite(osp.join(opts.out_path,'proc_mirr_'+osp.basename(img_path)), 255*np.transpose(img_m, (1, 2, 0)))
    ...
    if use_annotations:
        ...
        print(np.median(IOU))
        print(np.std(IOU))
        if opts.output_measure:
            iou_output = [opts.num_train_epoch,np.mean(IOU),np.median(IOU),np.std(IOU)]
            overlap_output = [opts.num_train_epoch,np.mean(overlap),np.median(overlap),np.std(overlap)]
            pck_output = [opts.num_train_epoch,np.mean(err_tot, axis=0),np.median(err_tot, axis=0),np.std(err_tot, axis=0)]
            with open('/user/work/gh18931/diss/output/IOU.csv', 'a',newline='') as f:
                writer = csv.writer(f)
                writer.writerow(iou_output)
            with open('/user/work/gh18931/diss/output/PCK.csv', 'a',newline='') as f:
                writer = csv.writer(f)
                writer.writerow(pck_output)
            with open('/user/work/gh18931/diss/output/Overlap.csv', 'a',newline='') as f:
                writer = csv.writer(f)
                writer.writerow(overlap_output)
